...
def Non_trivial_factor(N,B,Sign,depth,Old_dict,start_time) :
	...
	if Sign == "pos" :
		...
		depth+=1
		if B<1000 :
			return main(N,B+150,"pos_neg",depth,Old_dict,start_time)
		else :
			if depth < 999 :
...
def main(N=1769883798856697,B=20,Sign="pos",depth=1,Old_dict={},start_time=0):
	'''
...
time :	The program stops if the timeout is exceeded. if timeout == 0 , endless run (within depth limit).
	...
	timeout=0 #initializing the start_time
	if depth==1 :
		start_time=time()
	...
	if type(D)==dict :
		#printing
		#print("\n\t List of number found in step_1 {} -1 as factor :".format('without' if Sign=="pos" else 'including'))			
		#for m in D.keys():
		#	print(str(m)+'^2 - ',N, '\t= ',m**2-N,'\t= ',Classic_Writing(D[m]),sep='')
...
##############################################################################################################################################################
		...
		if A!=[] :
			...
			if len(A)>1:
				...
				print("\n\n\t Matrix of exponents mod(2) cleaned up :\n A =")
				for a in A :
					print('|',' '.join(list(map(lambda x : str(x),a))),'|')
				print("\nbase :",'(',' '.join(list(map(lambda x : str(x),f_base))),')')
		else :
...
##############################################################################################################################################################
		...
		if len(A)>1 :
			...
			A=weight(N,A,smooth_D,B,f_base,margin=0.2)
#			print(weights)
			print("\n\t Finally, we have the following numbers remaining :\nA =")
			for a in A :
					print('|',' '.join(list(map(lambda x : str(x),a))),'|')
			print("\nbase :",'(',' '.join(list(map(lambda x : str(x),f_base))),')')
...
##############################################################################################################################################################
		...
		for nb in Sol :
			print(nb,'{}'.format('*' if nb!=Sol[-1] else ''),sep='',end='')
		print(" )^2 \t\t= ", X,sep='')
		print("Let's Y \t= sqrt( ", Classic_Writing(D_Sol), " ) \t= ",Y, sep='')
...
#exemple de nombre non factorisable avec l'ancien algorithme : 213423564
